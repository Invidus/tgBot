import {
  getUserInfo,
  setFreeRequestsByUsername,
  setSubscriptionByUsername
} from './userService.js';
...
/**
 * Получает информацию о пользователе и форматирует для отправки
 */
 ...
 */
export const processGetUserInfo = async (ctx, text) => {
 ...
 */
export const processSetFreeRequests = async (ctx, text) => {
  ...
  try {
    const user = await setFreeRequestsByUsername(username, count);
 ...
 */
export const processSetSubscription = async (ctx, text) => {
  const parts = text.trim().split(/\s+/);
  ...
  try {
    const user = await setSubscriptionByUsername(username, days);
